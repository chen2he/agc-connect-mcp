...
import { registerKnowledgeTools, registerTools } from "./tools.js";
...
- 提交发布、回复评论、下架、删除等对外生效的操作，执行前先向用户确认。`;
...
async function main() {
  ...
  console.error(`[agc-mcp] 已启动：凭据=${client.auth.describe()}，地址=${config.baseUrl}${config.readOnly ? "，只读模式" : ""}${knowledgeEnabled ? "，已启用鸿蒙知识库" : ""}`);
}
